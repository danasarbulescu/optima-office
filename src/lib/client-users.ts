...
export async function updateClientUser(
  id: string,
  updates: Partial<Pick<ClientUser, 'firstName' | 'lastName' | 'email' | 'status' | 'authorizedPackageIds' | 'authorizedDashboardIds' | 'authorizedEntityIds' | 'defaultDashboardId' | 'cognitoUserId'>>,
): Promise<void> {
  if (!TABLE_NAME) throw new Error('CLIENT_USERS_TABLE not configured');
